...
export function Modalidades(){
    ...
    async function getModalidades(){
        try {
            const response = await fetch('http://localhost:8080/modalidades?activo='+ (activo ? 1:0))
    ...
    return(
        <Box>
            <Box display={'flex'} sx={{ height: 55}}>
                <TextField label={'Modalidades'} sx={{ height:40, flexGrow:1}} onChange={ (event) => {
            ...
            <Box sx={{mt:1}}>
                <TableContainer component={Paper}>
                    <Table sx={{ minWidth: 650 }} aria-label="simple table">
                        ...
                        <TableBody>
                            {modalidades.filter(({nombre}) => nombre.includes(filtro)).map(({id,nombre}) => (
                            <TableRow
                                key={id}
